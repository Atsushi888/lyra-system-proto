...
def _get_persona_json_dir() -> str:
    ...
    見つからなければ 'actors/persona_datas' を既定値とする。
    """
    base = None
...
class PersonaEditor:
    ...
    def render(self) -> None:
        ...
        if not json_files:
            st.error("Persona JSON が見つかりませんでした。"
        ...
        selected_file = st.selectbox(
            ...
            options=json_files,
            index=max(json_files.index(current_file) if current_file in json_files else 0, 0),
        )
        ...
        with col_p3:
            politeness_level = st.selectbox(
                ...
                index=["polite", "casual", "mix"].index(
                    str(data.get("politeness_level", "mix"))  # fallback
        ...
        with col_e1:
            baseline_affection = st.slider(
                ...
                0.0, 1.0, _get_emo("baseline_affection", 0.8),
                step=0.05,
            )
            baseline_arousal = st.slider(
                "baseline_arousal（デフォルトの感情の高まり）",
                0.0, 1.0, _get_emo("baseline_arousal", 0.4),
                step=0.05,
            )
            shyness = st.slider(
                "shyness（照れやすさ）",
                0.0, 1.0, _get_emo("shyness", 0.7),
                step=0.05,
            )
        with col_e2:
            jealousy = st.slider(
                "jealousy（嫉妬しやすさ）",
                0.0, 1.0, _get_emo("jealousy", 0.6),
                step=0.05,
            )
            anger_threshold = st.slider(
                "anger_threshold（怒りが表面化するまでのしきい値／高いほど怒りにくい）",
                0.0, 1.0, _get_emo("anger_threshold", 0.9),
                step=0.05,
            )
